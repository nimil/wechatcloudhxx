...
// Delete 删除点赞记录
func (dao *UserLikeDaoImpl) Delete(userId, postId string) error {
	return dao.db.Where("user_id = ? AND post_id = ?", userId, postId).Delete(&model.UserLikeModel{}).Error
}

// IsLiked 检查用户是否点赞
func (dao *UserLikeDaoImpl) IsLiked(userId, postId string) (bool, error) {
	var count int64
...
func (dao *UserLikeDaoImpl) GetUserLikedPostIds(userId string) ([]string, error) {
	var postIds []string
